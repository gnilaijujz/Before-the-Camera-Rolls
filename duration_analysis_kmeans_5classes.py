...
# 为每个类别设置名称
cluster_names = ['超短视频', '短视频', '中等视频', '长视频', '超长视频']
...
colors = ['#3274A1', '#E1812C', '#3A923A', '#D64550', '#9467BD']
bars = plt.bar(category_labels, cluster_stats['视频数量'], color=colors, width=0.6)

# 添加标题和标签
plt.title('视频按时长的KMeans聚类结果（5类）', fontsize=16, fontweight='bold', pad=20)
...
max_duration = min(df['duration_minutes'].max(), 50)  # 限制最大显示时长，防止极端值影响可视化
sns.kdeplot(data=df, x='duration_minutes', hue='cluster_name', fill=True, 
           alpha=0.5, linewidth=1.5, clip=(0, max_duration))

plt.title('视频时长聚类密度分布热力图', fontsize=16, fontweight='bold')
plt.xlabel('视频时长（分钟）', fontsize=14)
plt.ylabel('密度', fontsize=14)
