...
# Set gradient colors for bar chart - FIXED: now darker colors correspond to higher popularity
# Create a mapping based on popularity values, not positions
norm = plt.Normalize(duration_heat_mean['popularity_normalized'].min(), 
                    duration_heat_mean['popularity_normalized'].max())

# Use reversed colormap for bars to ensure highest value has darkest color
colors = plt.cm.YlOrRd(norm(duration_heat_mean['popularity_normalized']))

# Plot bars with correct color mapping
bars = plt.bar(
...
cbar = plt.colorbar(sm, ax=plt.gca(), label='Average Popularity')
