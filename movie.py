...
# -*- coding: utf-8 -*-
"""
movie_titleanalysis.py – end‑to‑end analysis of YouTube MOVIE / FILM video titles
Sibling script to gaming_titleanalysis.py, music_titleanalysis.py, titleanalysis.py.
...
# 2. Paths
# ==================================================
DATA_PATH  = "movie_2024-2025.csv"   # <-- your uploaded Movies dataset
OUTPUT_DIR = "movie_title_analysis"
...
def resolve_col(df, preferred, fallbacks, required=True):
    """
    Return the first matching column in df among preferred + fallbacks.
    Raise KeyError (with helpful message) if required and none found.
    """
...
COL_TITLE = resolve_col(df_raw, "title",
                        ["video_title", "name", "movie_title", "film_title"])
COL_PUB   = resolve_col(df_raw, "published_at",
                        ["publish_date", "published_date", "release_date",
                         "date", "upload_date"])
COL_DUR   = resolve_col(df_raw, "duration_seconds",
                        ["duration", "length_seconds", "length",
                         "runtime_seconds", "runtime"])
COL_POP   = resolve_col(df_raw, "popularity_normalized",
                        ["views_normalized", "views", "view_count",
                         "like_view_score", "engagement_score", "box_office_norm"])

# Standardise column names used downstream
...
# If popularity looks like raw counts (>2), normalise to 0‑1 range
if df["popularity_normalized"].max() > 2:
...
def plateau_decay(t, a=0.25, t0=20, floor=0.35):
    """
    Logistic‑plateau time weighting: recent uploads ~1.0,
    older uploads decay but never below 'floor'.
    """
...
# ==================================================
# Baseline English stop‑words + generic promo fluff + Film/Trailer boilerplate
STOP_WORDS = set(stopwords.words("english")).union({
    "subscribe","channel","follow","like","watch","video","official","hd",
    "4k","imax","ultra","new","trailer","teaser","clip","clips","scene","scenes",
    "featurette","behind","behindthescenes","bts","movie","film","films","full",
    "fullmovie","shortfilm","short","remastered","restored","extended","cut",
    "director","directors","fan","fanmade","promo","spot","tvspot","tv","part",
    "episode","ep","premiere","now"
})
...
# Save processed CSV early for inspection
df.to_csv(os.path.join(OUTPUT_DIR, "processed_movie_titles.csv"),
          index=False, encoding="utf-8-sig")

# ==================================================
# 6. TF‑IDF + LDA topics
# ==================================================
NUM_TOPICS = 5  # change if you'd like finer/coarser themes
...
def plot_topics(model, feat_names, n=10):
    ...
    fig.suptitle("Top words per LDA topic (Movies)")
    fig.tight_layout()
    return fig

plot_topics(lda, tfidf_vec.get_feature_names_out())\
    .savefig(os.path.join(OUTPUT_DIR, "movie_topic_keywords.png"))
...
for dur in ["short","medium","long"]:
    ...
    if len(sub) < 20:
        print(f"⚠️  Only {len(sub)} {dur} movie samples – model skipped.")
        continue
    ...
    rf.fit(X_tr,y_tr)
    joblib.dump(rf, os.path.join(OUTPUT_DIR, f"{dur}_movie_title_model.joblib"))
    ...
    plt.title(f"{dur.capitalize()} Movies – top features")
    plt.xlabel("Importance")
    plt.savefig(os.path.join(OUTPUT_DIR, f"{dur}_movie_feature_importance.png"))
    plt.close()
    ...
    plt.title(f"{dur.capitalize()} Movies – predicted vs actual")
    plt.xlabel("Actual"); plt.ylabel("Predicted")
    plt.savefig(os.path.join(OUTPUT_DIR, f"{dur}_movie_prediction_scatter.png"))
    plt.close()

# ==================================================
# 8. Word‑clouds (gradient + token filtering)
# ==================================================
...
lo = " ".join(df[df["adj_popularity"]<df["adj_popularity"].quantile(.25)]
              ["lemmatised_title"])
cloud(hi, "High popularity movie title keywords", "high_pop_movie_title_wc.png")
cloud(lo, "Low popularity movie title keywords",  "low_pop_movie_title_wc.png")
...
plt.title("Word count vs popularity (Movies)")
...
plt.title("Sentiment vs popularity (Movies)")
plt.xlabel("Sentiment bin"); plt.xticks(rotation=15)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "movie_overview_plots.png"))
plt.close()

# ==================================================
# 10. Text report
# ==================================================
with open(os.path.join(OUTPUT_DIR, "movie_title_analysis_report.txt"),
          "w", encoding="utf-8") as f:
    f.write("="*60+"\nMovie Title Impact on Popularity Report\n"+"="*60+"\n\n")
    f.write(f"Generated on: {datetime.now():%Y-%m-%d %H:%M:%S}\n")
    f.write(f"Total movie videos: {len(df)}\n")
    for d in ["short","medium","long"]:
        f.write(f"{d.capitalize()} movie videos: {len(df[df['duration_type']==d])}\n")
    f.write("\nModel performance:\n")
...
print(f"✅ Movie artefacts saved to '{OUTPUT_DIR}'")
