...
# -*- coding: utf-8 -*-
"""
gaming_titleanalysis.py – end‑to‑end analysis of YouTube GAMING video titles
Derived from music_titleanalysis.py / titleanalysis.py pipeline family.
...
# 2. Paths
# ==================================================
DATA_PATH  = "game_2024-2025.csv"   # <-- your uploaded Gaming dataset
OUTPUT_DIR = "gaming_title_analysis"
...
def plateau_decay(t, a=0.25, t0=20, floor=0.35):
    """
    Logistic‑plateau time weighting.
    Keeps recent uploads close to 1.0, decays older ones, never below floor.
    """
...
# ==================================================
# Baseline English stop‑words + Gaming‑specific fluff terms we usually down‑weight.
STOP_WORDS = set(stopwords.words("english")).union({
    "subscribe","channel","follow","like","watch","video",
    "instagram","facebook","twitter","tiktok","website","click","link",
    # Gaming fluff / promo words:
    "gameplay","walkthrough","let","lets","play","letsplay","lp",
    "live","stream","livestream","vod","highlights","clip","clips",
    "update","patch","dlc","season","new","ep","episode","part"
})
...
# Save the processed CSV early for inspection
df.to_csv(os.path.join(OUTPUT_DIR, "processed_gaming_titles.csv"),
          index=False, encoding="utf-8-sig")

# ==================================================
# 6. TF‑IDF + LDA topics
# ==================================================
NUM_TOPICS = 5   # adjust to taste
...
def plot_topics(model, feat_names, n=10):
    ...
    fig.suptitle("Top words per LDA topic (Gaming)")
    fig.tight_layout()
    return fig

plot_topics(lda, tfidf_vec.get_feature_names_out())\
    .savefig(os.path.join(OUTPUT_DIR, "gaming_topic_keywords.png"))
...
for dur in ["short","medium","long"]:
    ...
    if len(sub) < 20:
        print(f"⚠️  Only {len(sub)} {dur} gaming samples – model skipped.")
        continue
    ...
    rf.fit(X_tr,y_tr)
    joblib.dump(rf, os.path.join(OUTPUT_DIR, f"{dur}_gaming_title_model.joblib"))
    ...
    plt.title(f"{dur.capitalize()} Gaming – top features")
    plt.xlabel("Importance")
    plt.savefig(os.path.join(OUTPUT_DIR, f"{dur}_gaming_feature_importance.png"))
    plt.close()
    ...
    plt.title(f"{dur.capitalize()} Gaming – predicted vs actual")
    plt.xlabel("Actual"); plt.ylabel("Predicted")
    plt.savefig(os.path.join(OUTPUT_DIR, f"{dur}_gaming_prediction_scatter.png"))
    plt.close()
...
lo = " ".join(df[df["adj_popularity"]<df["adj_popularity"].quantile(.25)]
              ["lemmatised_title"])
cloud(hi, "High popularity gaming title keywords", "high_pop_gaming_title_wc.png")
cloud(lo, "Low popularity gaming title keywords",  "low_pop_gaming_title_wc.png")
...
plt.title("Word count vs popularity (Gaming)")
...
plt.title("Sentiment vs popularity (Gaming)")
plt.xlabel("Sentiment bin"); plt.xticks(rotation=15)

plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "gaming_overview_plots.png"))
plt.close()

# ==================================================
# 10. Text report
# ==================================================
with open(os.path.join(OUTPUT_DIR, "gaming_title_analysis_report.txt"),
          "w", encoding="utf-8") as f:
    f.write("="*60+"\nGaming Title Impact on Popularity Report\n"+"="*60+"\n\n")
    f.write(f"Generated on: {datetime.now():%Y-%m-%d %H:%M:%S}\n")
    f.write(f"Total gaming videos: {len(df)}\n")
    for d in ["short","medium","long"]:
        f.write(f"{d.capitalize()} gaming videos: {len(df[df['duration_type']==d])}\n")
    f.write("\nModel performance:\n")
...
print(f"✅ Gaming artefacts saved to '{OUTPUT_DIR}'")
