...
# 创建短视频和长视频的区分
df['video_length_category'] = df['initial_cluster'].map(
    {center_order_initial[0]: '短视频组', center_order_initial[1]: '长视频组'}
)

# 3) 对短视频组再次聚类为2类
# 筛选出短视频组的数据
short_videos = df[df['video_length_category'] == '短视频组']
if len(short_videos) > 0:
    ...
    # 在原始数据框中创建子类别
    df.loc[df['video_length_category'] == '短视频组', 'short_video_subclass'] = np.nan
    df.loc[short_videos.index, 'short_video_subclass'] = short_videos_clusters

    # 映射为超短视频和短视频
    mapping_short = {
        center_order_short[0]: '超短视频', 
        center_order_short[1]: '短视频'
    }
    df.loc[short_videos.index, 'final_category'] = df.loc[short_videos.index, 'short_video_subclass'].map(mapping_short)

# 4) 设置长视频组的类别（统一为"长视频"）
df.loc[df['video_length_category'] == '长视频组', 'final_category'] = '长视频'
...
# 添加标题和标签
plt.title('视频时长分类结果（超短视频、短视频、长视频）', fontsize=16, fontweight='bold', pad=20)
...
# 添加类别分界线
for name in category_order:
    if name != category_order[-1]:  # 不为最后一类添加上限线
...
plt.legend(handles=[
    plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=plt.cm.viridis(0/2), markersize=10, label='超短视频'),
    plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=plt.cm.viridis(1/2), markersize=10, label='短视频'),
